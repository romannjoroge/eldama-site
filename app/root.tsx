...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-surface-soft px-4 py-20">
      <div className="w-full max-w-md rounded-md border border-hairline bg-white p-8 text-center">
        <h1 className="text-3xl font-extrabold tracking-tight text-ink">
          {message}
        </h1>
        <p className="mt-3 text-body">{details}</p>
        {stack && (
          <pre className="mt-4 overflow-x-auto rounded-md bg-surface-soft p-4 text-left text-xs text-body">
            <code>{stack}</code>
          </pre>
        )}
      </div>
