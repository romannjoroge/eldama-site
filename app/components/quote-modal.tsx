  ...
  return (
    <QuoteContext.Provider value={{ openQuote, closeQuote }}>
      ...
      {open && (
        ...
        >
          <div
            className="fixed inset-0 bg-navy-950/50"
            onClick={closeQuote}
            aria-hidden="true"
          />
          <div className="flex min-h-full items-center justify-center p-4 sm:p-6">
            <div
              ref={panelRef}
              tabIndex={-1}
              className="relative w-full max-w-xl rounded-md border border-hairline bg-white p-6 outline-none sm:p-8"
            >
              <button
                type="button"
                onClick={closeQuote}
                aria-label="Close quote form"
                className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full text-mute transition-colors hover:bg-surface-soft hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              >
